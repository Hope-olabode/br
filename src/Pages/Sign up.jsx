...
export default function Signup() {
  ...
  const name = watch("firstname"); // watch input value
  const name2 = watch("lastname"); // watch input value
  const password = watch("password"); // watch input value
  const password2 = watch("password2"); // watch input value

  useEffect(() => {
    setIsFocused3(name && name.trim().length > 0);
  }, [name]);
  useEffect(() => {
    setIsFocused4(name2 && name2.trim().length > 0);
  }, [name2]);
    ...
    <div className="mt-[96px] py-32 px-4 flex flex-col content-center items-center">
      <div className="max-w-[482px]">
        ...
        <form className="mt-10 lg:mt-14" onSubmit={handleSubmit(onSubmit)}>
          ...
          <button type="submit" className="bg-[#E2063A] mt-4 text-white  rounded-full relative overflow-hidden group lg:h-[72px] lg:w-full  w-[100%]" disabled={password?.trim()?.length === 0 || email?.trim()?.length === 0 || password2?.trim()?.length === 0 || name?.trim()?.length === 0 || name2?.trim()?.length === 0}>
            <div className={`${password?.trim()?.length === 0 || email?.trim()?.length === 0 || password2?.trim()?.length === 0 || name?.trim()?.length === 0 || name2?.trim()?.length === 0 ? "inset-0 bg-[#ffffffd0] z-10 absolute w-100%" : ""} relative  px-4 py-[13px] lg:py-[23px] lg:px-0  `}>
              <span class="relative z-10 "><p className="font-nexa-bold text-[14px] leading-[22px] text-left lg:text-[16px] lg:leading-[26px] lg:pl-[40px]">Create Account</p></span>
              <div className="absolute right-[10px] top-[50%] translate-y-[-50%] lg:right-[25px]">
                <img src={wc} className={`${password?.trim()?.length === 0 || email?.trim()?.length === 0 || password2?.trim()?.length === 0 || name?.trim()?.length === 0 || name2?.trim()?.length === 0 ? "hi" : "hidden"} lg:h-10`}/>
                <img src={ba} className={`${password?.trim()?.length === 0 || email?.trim()?.length === 0 || password2?.trim()?.length === 0 || name?.trim()?.length === 0 || name2?.trim()?.length === 0 ? "hidden" : "block"} lg:h-10`}/>
              </div>
            </div>
          </button>
